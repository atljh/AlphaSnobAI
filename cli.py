...
def start(
    ...
    try:
        if foreground:
            ...
            if pid > 0:
                show_success(f"Bot started successfully!")
                show_info(f"PID: {pid}")
                show_info(f"Status: python cli.py status")
                show_info(f"Logs: python cli.py logs")
            else:
                import main
                main.main()
...
def restart():
    ...
    try:
        ...
        if pid > 0:
            show_success(f"Bot restarted successfully!")
...
@app.command()
def stats(
    chat_id: Optional[int] = typer.Argument(None, help="Chat ID to show stats for")
):
...
def main_callback(ctx: typer.Context):
    if ctx.invoked_subcommand is None:
        ...
        console.print("  [green]start[/green]    - Start the bot daemon")
        ...
        console.print("  [green]stats[/green]    - Show statistics")
        console.print("  [green]version[/green]  - Show version info")
        console.print("\n[dim]Use 'python cli.py COMMAND --help' for more info[/dim]\n")
