...
class Settings:
    def __init__(self, config_path: Path | None = None, secrets_path: Path | None = None):
    ...
    def _parse_persona_config(self, config: dict[str, Any]):
        ...
        self.persona = PersonaConfig(
            ...
            owner=PersonaSettings(
                corpus_path=self.base_dir
                / owner_cfg.get("corpus_path", "data/owner_samples/messages.txt"),
    ...
    def _parse_decision_config(self, config: dict[str, Any]):
        ...
        relationship_multipliers = decision_config.get(
        ...
        self.decision = DecisionConfig(
            ...
            topic_based=TopicBasedConfig(
                ...
                interesting_topic_multiplier=float(
                    topic_based_cfg.get("interesting_topic_multiplier", 1.5),
                ),
            ),
            context_aware=ContextAwareConfig(
                enabled=bool(context_aware_cfg.get("enabled", True)),
                recent_response_cooldown_seconds=int(
                    context_aware_cfg.get("recent_response_cooldown_seconds", 60),
                ),
                max_consecutive_responses=int(
                    context_aware_cfg.get("max_consecutive_responses", 3),
                ),
                consecutive_response_multiplier=float(
                    context_aware_cfg.get("consecutive_response_multiplier", 0.5),
                ),
            ),
            cooldown=CooldownConfig(
                enabled=bool(cooldown_cfg.get("enabled", True)),
                min_seconds_between_responses=int(
                    cooldown_cfg.get("min_seconds_between_responses", 30),
                ),
    ...
    def _parse_profiling_config(self, config: dict[str, Any]):
        ...
        self.profiling = ProfilingConfig(
            ...
            trust_adjustment=TrustAdjustmentConfig(
                positive_markers=trust_adjustment_cfg.get(
                    "positive_markers",
                    ["thanks", "спасибо"],
                ),
    ...
    def _parse_owner_learning_config(self, config: dict[str, Any]):
        ...
        self.owner_learning = OwnerLearningConfig(
            ...
            collection_path=self.base_dir
            / owner_config.get("collection_path", "data/owner_samples/collected.txt"),
            manual_samples_path=self.base_dir
            / owner_config.get("manual_samples_path", "data/owner_samples/messages.txt"),
    ...
    def validate(self) -> list[str]:
        ...
        if self.llm.temperature > 1.5:
            warnings.append(
                f"High temperature ({self.llm.temperature}) may produce very random responses",
            )
